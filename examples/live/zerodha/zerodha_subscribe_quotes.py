...
# -------------------------------------------------------------------------------------------------
"""
Zerodha Python TradingNode smoke test: subscribes to live quote ticks for one NSE equity
and prints the first 10 ticks that arrive.

Prerequisites:
- ZERODHA_API_KEY / ZERODHA_API_SECRET / ZERODHA_ACCESS_TOKEN in env (run
  ``python examples/live/zerodha/login.py`` first to refresh the daily token).
- NSE market hours: 09:15-15:30 IST Mon-Fri (excluding NSE holidays).

Override the instrument via ZERODHA_INSTRUMENT_ID, e.g.::

    ZERODHA_INSTRUMENT_ID="TCS-EQ.NSE" \\
